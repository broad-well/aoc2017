...
const INPUT: &'static str = "3458	3471	163	1299	170	4200	2425	167	3636	4001	4162	115	2859	130	4075	4269
...
fn into_table() -> Vec<Vec<i32>> {
    INPUT
            .lines()
            .map(|line| line.split('\t'))
            .map(|row| row.map(|num| num.parse::<i32>().unwrap()).collect())
            .collect()
}

fn part1() {
    let result = into_table().iter() 
            .map(|row| row.iter().max().unwrap_or(&0) - row.iter().min().unwrap_or(&0)).sum::<i32>();
    println!("{}", result);
}
