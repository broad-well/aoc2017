const INPUT: &'static str = "sayndz zfxlkl attjtww cti sokkmty brx fhh suelqbp
...
fn part1(is_part2: bool) -> usize {
    INPUT
        ...
        .map(|mut line| {
            if is_part2 {
                line.iter_mut().for_each(|word| {
                    ...
                    word.clone_from(&String::from_iter(vec));
                });
            }
            line.sort();
            line
        ...
        .filter(|line| {
            ...
            unique.dedup();
            unique == *line && unique.len() == line.len()
        })
        .count()
}

fn main() {
    println!("{} {}", part1(false), part1(true));
}